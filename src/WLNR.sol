...
contract WLNR is ERC721 {
    ...
    function deposit(string calldata _name, bytes4 _lang, bytes4 _type) external {
        ...
        require(NAME[id]._wrapped == 0, "ALREADY_WRAPPED");
        require(LNR.owner(name32) == address(this), "OWNER_NOT_CHANGED");
        require(LNR.addr(name32) == msg.sender, "DEPOSIT_ADDR_NOT_SET"); 
        address _block = blocks[_lang][_type];
        if (_block == address(0)) revert InvalidLangType(_lang, _type);
        if (NAME[id]._lang == bytes4(0)) {
            ...
            //NH2ID[keccak256(abi.encodePacked(bytes32(0), keccak256(bytes(_name))))] = id;
    ...
    function checkName(string calldata _name, bytes4 _lang, bytes4 _type) public view returns(uint8 len) {
        ...
        if (!ok || abi.decode(_result, (bool))) revert InvalidName(string(_name), _lang, _type);
        len = abi.decode(_result, (uint8));
        require(len != 0, "INVALID_FORMAT");
    }
    //(bool ok, bytes memory _result) = _block.staticcall(abi.encodeWithSelector(_type, bytes(name))); 
    //uint8 len = 0;
    //if(!ok || (len = abi.decode(_result, (uint8))) == 0) 
    //    revert InvalidName(_name, _lang, _type);
    ...
    mapping(bytes32 => uint) public NH2ID;
    ...
    function tokenURI(uint256 id) public view returns(string memory) {
        ...
        if (_n._lang == bytes4(0)) revert NotWrapped(string(abi.encodePacked(id)));
    ...
    function royaltyInfo(uint256 id, uint256 _salePrice) external view returns(address to, uint256 amount) {
        ...
        // pseudo random, not safe /check timestamp
    ...
    /**
     * @dev : transfer contract ownership to new multisig
