...
abstract contract ResolverBase {
    ...
    modifier onlyDev() {
        if (msg.sender != WLNR.Dev()) {
            revert OnlyDev(WLNR.Dev(), msg.sender);
        }
        _;
    }

    /// @dev : LNR Contract Interface
    iLNR public LNR;

    /// @dev : WLNR Contract Interface
    iWLNR public WLNR;

    mapping(bytes4 => bool) public supportsInterface;

    modifier isValidToken(uint256 id) {
        if (id >= WLNR.totalSupply()) {
    ...
    function withdrawEther() external payable {
        (bool ok,) = WLNR.Dev().call{value: address(this).balance}("");
...
contract Resolver is ResolverBase {
    ...
    constructor(address _wlnr) {
        WLNR = iWLNR(_wlnr);
        LNR = iLNR(0x00000000000C2E074eC69A0dFb2997BA6C7d2e1e);
    ...
    function addr(bytes32 node, uint256 coinType) external view returns (bytes memory _addr) {
        ...
        if (_addr.length == 0 && coinType == 60) {
            _addr = abi.encodePacked(LNR.owner(node));
        }
    }
